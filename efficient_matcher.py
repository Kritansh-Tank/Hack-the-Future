...
class EfficientMatcher:
    ...
    def __init__(self, embeddings_cache_dir=None, use_parallel=True, max_workers=4, batch_size=10,
        ...
        self.embeddings_cache_dir = embeddings_cache_dir or os.path.join(config.BASE_DIR, "cache", "embeddings")
        ...
        if self.use_parallel:
            logger.info("Using parallel processing - thread-local database connections will be created as needed")
        
        logger.info("Initialized EfficientMatcher with performance optimizations")
        ...
        if os.path.exists(cache_path):
            try:
                with open(cache_path, 'rb') as f:
                    ...
                    logger.debug(f"Loaded {type_prefix} embedding from cache: {item_id}")
                    return embedding
    ...
    def precompute_job_embeddings(self, job_ids=None):
        ...
        missing_job_ids = [jid for jid in job_ids if jid not in self.job_embeddings]
        
        if missing_job_ids:
            logger.info(f"Computing embeddings for {len(missing_job_ids)} jobs...")
        ...
        logger.info(f"Job embeddings ready for {len(self.job_embeddings)} jobs")
    ...
    def _compute_job_embedding(self, job_id):
        ...
        try:
            ...
            # Generate embedding
            embedding = self.embedding_util.get_embedding(job_text, use_cache=False)
            ...
            logger.error(f"Error computing job embedding for job {job_id}: {str(e)}")
            return None
    ...
    def precompute_candidate_embeddings(self, candidate_ids=None):
        ...
        for candidate_id in candidate_ids:
            cached_embedding = self._load_cached_embedding('candidate', candidate_id)
            if cached_embedding is not None:
                self.candidate_embeddings[candidate_id] = cached_embedding
        
        # Compute missing embeddings
        missing_candidate_ids = [cid for cid in candidate_ids if cid not in self.candidate_embeddings]
        
        if missing_candidate_ids:
            logger.info(f"Computing embeddings for {len(missing_candidate_ids)} candidates...")
        ...
        logger.info(f"Candidate embeddings ready for {len(self.candidate_embeddings)} candidates")
    ...
    def _compute_candidate_embedding(self, candidate_id):
        ...
        try:
            ...
            # Generate embedding
            embedding = self.embedding_util.get_embedding(candidate_text, use_cache=False)
            ...
            logger.error(f"Error computing candidate embedding for candidate {candidate_id}: {str(e)}")
            return None
    ...
    def compute_initial_matches(self, job_id, candidate_ids=None, top_k=None):
        ...
        if candidate_ids is None:
            ...
            thread_local_db = DBManager()
            candidates = thread_local_db.db.cursor.execute('SELECT id FROM candidates').fetchall()
            candidate_ids = [c['id'] for c in candidates]
            thread_local_db.close()
        ...
        missing_candidates = [cid for cid in candidate_ids if cid not in self.candidate_embeddings]
    ...
    def _enhance_match_with_llm(self, job_data, match):
        ...
        try:
            ...
            overall_match = (skills_match * 0.5) + (quals_match * 0.3) + (match['similarity_score'] * 0.2)
            
            # Update scores
            enhanced_match['skills_match'] = skills_match
            enhanced_match['qualifications_match'] = quals_match
            enhanced_match['experience_match'] = match['similarity_score']  # Use similarity as experience match
    ...
    def _traditional_matching(self, job_id, candidate_ids, local_db=None, save_to_db=True):
        ...
        """
        logger.info(f"Using traditional matching for job {job_id} (LLM and embeddings disabled)")
        
        # Get job data for traditional matching
        job_data = (local_db or self.db_manager).get_job_by_id(job_id)
        job_text = job_data.get('jd_text', '') + ' ' + job_data.get('summary', '')
        ...
        for candidate in tqdm(all_candidates, desc=f"Matching job {job_id} with candidates (traditional)"):
            ...
            match_score = self.embedding_util.calculate_text_similarity(job_text, candidate_text)
        ...
        if save_to_db:
            self._save_match_results_to_db(matches, local_db or self.db_manager)
    ...
    def match_job_with_all_candidates(self, job_id, candidate_ids=None, enhance_top_k=None, save_to_db=True):
        ...
        try:
            ...
            if not getattr(config, 'USE_LLM', True) and not getattr(config, 'USE_EMBEDDINGS', True):
                matches = self._traditional_matching(job_id, candidate_ids, local_db, save_to_db)
                elapsed = time.time() - start_time
                logger.info(f"Matched job {job_id} with {len(candidate_ids)} candidates in {elapsed:.2f} seconds (traditional matching only)")
                return matches
            ...
            initial_matches = self.compute_initial_matches(job_id, candidate_ids)
            
            # Enhance top matches with LLM if requested
            if getattr(config, 'USE_LLM', True) and enhance_top_k > 0 and initial_matches:
                enhanced_matches = self.enhance_matches_with_llm(job_id, initial_matches, enhance_top_k)
            else:
                ...
                if enhance_top_k > 0 and getattr(config, 'USE_LLM', True):
                    ...
                    logger.info(f"LLM enhancement disabled, skipping for job {job_id}")
                    
            # Save to database if requested
            if save_to_db and enhanced_matches:
                self._save_match_results_to_db(enhanced_matches, local_db or self.db_manager)
                
            elapsed = time.time() - start_time
            logger.info(f"Matched job {job_id} with {len(candidate_ids)} candidates in {elapsed:.2f} seconds")
    ...
    def match_jobs_with_candidates(self, job_ids=None, candidate_ids=None, enhance_top_k=None, save_to_db=True):
        ...
        if job_ids is None:
            ...
            thread_local_db = DBManager()
            jobs = thread_local_db.db.cursor.execute('SELECT id FROM job_descriptions').fetchall()
            job_ids = [j['id'] for j in jobs]
            thread_local_db.close()
    ...
    def _save_match_results_to_db(self, matches, db_manager):
        ...
        for i in range(0, len(matches), self.batch_size):
            ...
            for match in batch:
                ...
                match_score = match.get('overall_match', match.get('match_score', 0))
                skills_match = match.get('skills_match', match.get('similarity_score', 0.5))
                quals_match = match.get('qualifications_match', 0.5)
                exp_match = match.get('experience_match', 0.5)
                notes = match.get('notes', match.get('match_notes', f"Match score: {match_score:.2f}"))
...
if __name__ == "__main__":
    ...
    try:
        # Get a job
        job = matcher.db_manager.db.cursor.execute('SELECT id FROM job_descriptions LIMIT 1').fetchone()
        ...
        if job:
            ...
            for i, match in enumerate(matches[:5]):
                ...
                print(f"   Notes: {match.get('notes', 'No notes available')[:100]}...")
        else:
